...
combined_data_mean_N <- combined_data |>   
  mutate(Month_Year = paste(month(Sample_Date), year(Sample_Date))) |> #create unique month - year column
  mutate(Month_Year = my(Month_Year)) |>   #extract month - year from that to get back into date format
  group_by(Month_Year, Sample_ID) |>   #group by month and location to create mean values
  summarize(Monthly_Mean = mean(`NO3-N`, na.rm = TRUE))   # -- create table of monthly means for our plot later 
...
ggplot(q2_allsites_K, aes(x = Month_Year , y = mean)) + geom_line(aes(linetype = Sample_ID,color = Sample_ID)) + scale_color_paletteer_d("fishualize::Koumansetta_rainfordi") + 
  labs(x = "Year",
       y = "Mean Potassium Concentration") +
  guides(color = guide_legend(title = "Sample Site"), linetype = guide_legend(title = "Sample Site")) +
  ggtitle("Mean Monthly Potassium Levels in Watersheds in the Luquillo Experimental Forest Before and After Hurricane Maria") + 
...
ggplot(q1_allsites_NO3, aes(x = Month_Year , y = mean)) + geom_line(aes(linetype = Sample_ID,color = Sample_ID)) + scale_color_paletteer_d("fishualize::Koumansetta_rainfordi")  + 
  labs(x = "Year",
       y = "Mean Nitrate Concentration") +
  guides(color = guide_legend(title = "Sample Site"), linetype = guide_legend(title = "Sample Site"))  +
  ggtitle("Mean Monthly Nitrate Levels in Watersheds in the Luquillo Experimental Forest Before and After Hurricane Maria") + 
     theme(plot.title = element_text(lineheight=.8, face="bold"))
```
